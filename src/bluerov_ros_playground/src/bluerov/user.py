    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                # Surge = Channel 5
                override[4] = int(joy[4]*400 + 1500)
                # Sway = Channel 6
                override[5] = int(-joy[3]*400 + 1500)
                # Yaw = Channel 4
                override[3] = int(joy[0]*400 + 1500)
                # Heave = Channel 3
                override[2] = int(joy[1]*400 + 1500)
                # Cam up = Channel 8
                override[7] = 1500+int(but[4]*400)-int(but[5]*400)
                # Converting button presses to roll and pitch setting modification
                curr_pitch_setting = enforce_limit(curr_pitch_setting - joy[7]*0.1)
                curr_roll_setting = enforce_limit(curr_roll_setting - joy[6]*0.1)
                if (but[4] == 1):
                    curr_pitch_setting = 0.0
                if (but[5] == 1):
                    curr_roll_setting = 0.0                    
                # Pitch = Channel 1
                override[0] = int(curr_pitch_setting*400 + 1500)
                # Roll = Channel 2
                override[1] = int(curr_roll_setting*400 + 1500)
                # Unused:
                # Ch 1 = Pitch
                # Ch 2 = Roll
                # Ch 7 = ?
